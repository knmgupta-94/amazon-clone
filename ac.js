...
let imgItem = document.querySelectorAll(".image-item");  // Use consistent casing
...
function handleLeft() {
    // Only allow left slide if we're not already at the first image
    if (startSlider < 0) {
...
function handleRight() {
    // Only allow right slide if we're not already at the last image
...
sidebaropennavigationEl.addEventListener("click", () => {
    sidebarnavigationEl.style.transform= `translateX(${value}%)`; // Correctly reference the variable and use camelCase for zIndex
});
...
let startproduct = 0;
let productWidth = 525;  // Assuming each product item has a width of 525px

// Calculate the total width of all items for correct sliding.
...
today_deal_btn_prevEl.addEventListener("click", () => {
    ...
    // Only allow sliding left if we're not at the first item.
    if (startproduct < 0) {
        startproduct += productWidth;  // Move to the right (previous item)
    }
...
today_deal_btn_nextEl.addEventListener("click", () => {
    ...
    // Only allow sliding right if we're not at the last item.
    if (startproduct > maxSlide) {
        startproduct -= productWidth;  // Move to the left (next item)
    }
...
let startproduct1 = 0;
let productWidth1 = 525;  // Assuming each product item has a width of 525px
...
today_deal1_btn_prevEl.addEventListener("click", () => {
    ...
    // Only allow sliding left if we're not at the first item.
    if (startproduct1 < 0) {
        startproduct1 += productWidth1;  // Move to the right (previous item)
    }
...
today_deal1_btn_nextEl.addEventListener("click", () => {
    ...
    if (startproduct1 > maxSlide1) {
        startproduct1 -= productWidth1;  // Move to the left (next item)
    }
